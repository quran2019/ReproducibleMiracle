...
\hypertarget{htmlwidget-14dbc4060e11c1cab4d4}{}
...
\hypertarget{htmlwidget-7cf7d9af3302ba0950a9}{}
...
\hypertarget{htmlwidget-704f3ad46f0f35f997e6}{}
...
\hypertarget{htmlwidget-80cf9197b53a62d936f2}{}
...
\item
  \textbf{The number of letters in numbered verses of Quran is \(330709\)} (denoted as \textbf{l}). See Chapter \ref{textTMlL} for the proof.
\item
  \textbf{The number of letters in Quran (including unnumbered verses) is \(332837\)} (denoted as \textbf{L}). See Chapter \ref{textTMlL} for the proof.
\end{enumerate}
...
\hypertarget{htmlwidget-541250dbeb9ac4a148b0}{}
...
\hypertarget{htmlwidget-f08bbef456a2b851f1fd}{}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Because we apply only the default test to}
\CommentTok{# the single number in this evidence.}
\NormalTok{global_tested_nums <-}\StringTok{ }\NormalTok{global_tested_nums }\OperatorTok{+}\StringTok{ }\DecValTok{1} 
\NormalTok{global_success_nums <-}\StringTok{ }\NormalTok{global_success_nums }\OperatorTok{+}\StringTok{ }\DecValTok{1}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## [1] "The probability of 7 success out of 10 (with p=1/19) is: 1.16554114725275e-07"
\end{verbatim}
...
\hypertarget{htmlwidget-f0d7f4d05775b773b0a4}{}
...
\hypertarget{htmlwidget-1016bb8807f943a648d7}{}
...
\hypertarget{htmlwidget-10c84d0533964072fb1b}{}
...
\hypertarget{EvAllahPositions}{%
\section{Evidence on Allah word positions and frequencies}\label{EvAllahPositions}}

In this evidence, I will present evidences that proves the positions and number of usage of the word God, in Arabic \textbf{الله} (\textbf{Allah}),
has its own 19 based design. These evidences prove that the distribution of the name of God (in Arabic, Allah) is not random and under precise 19 based design. These evidences suggest that the positions of the word Allah in the text of Quran stands as if a backbone within verses. These evidences prove that \textbf{Allah} words in Quran must be in their current locations and also in their current frequency in each verse. This observations further support the main hypothesis of this book that the text of Quran is intact and unchanged and it is under 19 based design.
...
\hypertarget{htmlwidget-898af0e27e95875e99a7}{}
...
\hypertarget{htmlwidget-f499c87d7a499d7e8440}{}

For the reference, to the best of my knowledge, All the evidences presented in the subsections of this Evidence \ref{EvAllahPositions} is first time presented to the literature in this book, and it has been hypothesized, tested and discovered by myself. In case if I find out that the evidences under Evidence \ref{EvAllahPositions} had been available in another article, then, in such a case, I surely add a citation and update the online version of this book.

\hypertarget{EvAllahPositionsiboth}{%
\subsection{Evidence on Allah word positions and frequencies in both text types}\label{EvAllahPositionsiboth}}

In this evidence, I will prove that the \textbf{chapter index}, \textbf{verse index} and \textbf{Allah word counts} are under 19 based design in the combination of both of the text types of Quran. It means, the design is over both text at the same time! Basically, I will concatenate all the numbers in natural order as I did in this book earlier for some other evidences. This means any single shift of any of those numbers might break the 19 based design. This proves that the positions of Allah word are under 19 based designed in the combination of both text types of Quran.
...
\url{https://github.com/quran2019/Quran19/blob/master/Evidence_7.3.1_bignumber.txt}

One can just copy the big number and paste it into a web based big number calculator and test it herself.

I will also compute the probability of evidences with respect to Allah words separately. In order to evaluate whether our observations on Allah word positions are by chance alone or by design, we have to compute the probability of our observations on these particular evidences. Therefore, I will count the number of trials and successful observations just for the evidences about Allah word positions.

For the particular evidence of this sub-section, we could also pair vectors the other way around. Therefore, in total, for this particular evidence, we have two tests and one success. We add them to the particular counters of the evidences about Allah word positions. We will use them in the end to compute the overall probability of our observations about Allah word positions.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#counters to compute the overall probability}
\NormalTok{AllahWordEvidenceTrials <-}\StringTok{ }\DecValTok{2}
\NormalTok{AllahWordEvidenceSuccesses <-}\StringTok{ }\DecValTok{1}
\end{Highlighting}
\end{Shaded}
...
\subsection{Evidence on Allah word positions and frequencies with sum}\label{EvAllahPositionwithsum}}

In this evidence, I will prove that the chapter indices, verse indices and Allah word counts are also under 19 based design in the numbered verses of Quran with respect to their total sum as follows. In the table of Allah word positions in 6236 verses' above, I provided the chapter indices, verse indices in that chapter and Allah word counts in that verse.
...
This further proves the positions and frequencies of Allah words are under 19 based design together in the numbered verses of Quran. We could also try this coding in all the 6348 verses. Since there is no order in the number, assuming that we also test the digit sum of them, therefore we have 4 trials and one success in this case. Let's add them to the counters as follows.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#counters to compute the overall probability}
\NormalTok{AllahWordEvidenceTrials <-}\StringTok{ }\NormalTok{AllahWordEvidenceTrials }\OperatorTok{+}\DecValTok{4}
\NormalTok{AllahWordEvidenceSuccesses <-}\StringTok{ }\NormalTok{AllahWordEvidenceSuccesses }\OperatorTok{+}\StringTok{ }\DecValTok{1}
\end{Highlighting}
\end{Shaded}

\hypertarget{EvAllahPositionwtConcat}{%
\subsection{Evidence on Allah word positions and frequencies with concatenation}\label{EvAllahPositionwtConcat}}
...
\url{https://github.com/quran2019/Quran19/blob/master/Evidence_7.3.3_big_number.txt}

One can just copy the big number and paste it into a web based big number calculator and test it herself.

A similar pattern also exist with the \textbf{special verse indices} of each chapter. However, this time the digit sum of the big number is multiple of 19. Therefore, with respect to general rule, I consider this as a \textbf{fact} rather than evidence. In this \textbf{fact}, we concatenate the chapter index, the verse index and the number of Allah words in each verse. For example, the first Allah word in the numbered verse of Basmala verse is in the first chapter (Al-Fatiha). Thus, we get this representative number for it after concatenating in the natural order: 111. Similarly, the following verse with Allah word is in the second chapter and special verse 7 with one Allah word. Thus, we get this representative number for it after concatenating in the natural order: 271. This is done for all the verses with Allah words in it. Then, we concatenate all of these representative numbers with each other in the normal order of the table. Then, we get 7298 digits length big number and the digit sum of this big number is \textbf{24586} and it is multiple of 19. Here is the proof of this test:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y <-}\StringTok{ }\NormalTok{ynQ }\CommentTok{# from 6236 verses}
\NormalTok{tmp<-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(y}\OperatorTok{$}\NormalTok{Chapter,}
\NormalTok{             y}\OperatorTok{$}\NormalTok{verse,y}\OperatorTok{$}\NormalTok{countW)}
\CommentTok{# head side of numbers of each verse}
\KeywordTok{head}\NormalTok{(tmp)}
...
\url{https://github.com/quran2019/Quran19/blob/master/Fact_7.3.3_bignumber.txt}

One can just copy the big number and paste it into a web based big number calculator and test it herself.

Since there is order in the number, we had two trials and one success. However, I will also take into account for the two cases as not successes though one of their digit sum is divisible by 19. Therefore, we have 4 trials and one success in these two cases. Let's add them to the counters as follows.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#counters to compute the overall probability}
\NormalTok{AllahWordEvidenceTrials <-}\StringTok{ }\NormalTok{AllahWordEvidenceTrials }\OperatorTok{+}\StringTok{ }\DecValTok{4}
\NormalTok{AllahWordEvidenceSuccesses <-}\StringTok{ }\NormalTok{AllahWordEvidenceSuccesses }\OperatorTok{+}\StringTok{ }\DecValTok{1}
\end{Highlighting}
\end{Shaded}

\hypertarget{EvAllahPositionVerseIndex}{%
\subsection{Evidence on Allah word positions and frequencies on verse indices only}\label{EvAllahPositionVerseIndex}}

In this evidence, I will prove that the general verse indices and the special verse indices of each chapter, which includes Allah words are also under 19 based design, are under 19 based design individually. Basically, we will only look at verse indices alone. There is one \textbf{evidence} based on general verse indices and one \textbf{fact} based on special verse indices in each chapter.

Evidence tests whether the general verse indices, in which there is Allah words has 19 based design. As we will see there is design in the numbered verses. When we concatenate each general verse index in the 6236 numbered verses, the resulting big number is multiple of 19. This clearly shows the positions of verses with Allah word in the numbered verses and we witness that it is under the 19 based designed and not random.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y <-}\StringTok{ }\NormalTok{ynQ }\CommentTok{#from numbered verses}
...
\url{https://github.com/quran2019/Quran19/blob/master/Evidence_7.3.4_big_number.txt}

One can just copy the big number and paste it into a web based big number calculator and test it herself.

One more \textbf{interesting fact} on this big number is that the length of this big number is 5719 and it is also multiple of 19 as 19x301.

\textbf{Fact:}

In the above evidence, we tested the general verse indices.
I also test the corresponding special verse indices of each chapter with the same pattern. In interesting enough, this is also under 19 based design as a fact that supports the above evidence.
When we concatenate each special verse index in the 6236 numbered verses, the digit sum of the resulting big number is 12730 and it is multiple of 19 as 19x670. This clearly shows the positions of verses with Allah word in the numbered verses and we witness that it is under the 19 based designed and not random.
...
\url{https://github.com/quran2019/Quran19/blob/master/Fact_7.3.4_big_number.txt}

Since there is order in the numbers, we had two trials and one success. However, I will also take into account for the two cases, considering the fact, as not successes since they are not directly divisible by 19, though digit sum is. Therefore we have 4 trials and one success in these two cases. Let's add them to the counters as follows.
...
\hypertarget{EvAllahPositionChapSpecVerses}{%
\subsection{Evidence on Allah word positions and frequencies with Chapter and Special verse indices}\label{EvAllahPositionChapSpecVerses}}

In this evidence, I will prove that the chapter indices and the special verse indices in each chapter with Allah words in it are under 19 based design in the numbered verses of Quran.

This evidence concatenates each chapter index and special verse index of each chapter with Allah words in it over all the 5236 numbered verses. The resulting 5732 digits length big number is multiple of 19. This shows that the positions of verses with Allah word in it are under 19 based design too. Therefore, we witness that it is under the 19 based designed and not random.
...
\url{https://github.com/quran2019/Quran19/blob/master/Evidence_7.3.5_big_number.txt}

One can just copy the big number and paste it into a web based big number calculator and test it herself.

Since there is order in the numbers, we had two trials and one success. Therefore we have 2 trials and one success in these two cases. Let's add them to the counters as follows.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#counters to compute the overall probability}
\NormalTok{AllahWordEvidenceTrials <-}\StringTok{ }\NormalTok{AllahWordEvidenceTrials }\OperatorTok{+}\StringTok{ }\DecValTok{2}
\NormalTok{AllahWordEvidenceSuccesses <-}\StringTok{ }\NormalTok{AllahWordEvidenceSuccesses }\OperatorTok{+}\StringTok{ }\DecValTok{1}
\end{Highlighting}
\end{Shaded}

\hypertarget{EvAllahPositionGenChapSpecVerses2}{%
\subsection{Evidence with General verse indices, Chapter and Special verse indices}\label{EvAllahPositionGenChapSpecVerses2}}

In this evidence, I will prove that the general verse indices, the chapter indices and the special verse indices in each chapter with Allah words in it are under 19 based design in all the verses.

This evidence concatenates each general verse index, chapter index and special verse index that have Allah words in it considering all the 6348 verses. We then concatenate each of these represenation numbers in its natural order. The resulting 12126 digits length big number is multiple of 19. This clearly shows the positions of verses with Allah word in it are under 19 based design and not random.
...
\url{https://github.com/quran2019/Quran19/blob/master/Evidence_7.3.6_big_number.txt}
...
\begin{verbatim}
## The probability of all the  6 success out of all the 18  tests (with p=1/19) is: 0.000227481815830633
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  6 success out of all the 18  tests (with p=1/19) and Probability of number 19
##   being in Quran together is: 1.99545452483011e-06
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  6 success out of all the 36  tests (with p=1/19) is: 0.0105952854522913
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  6 success out of all the 36  tests (with p=1/19) and Probability of
##   number 19 being in Quran together is: 9.29411004586953e-05
\end{verbatim}
...
\hypertarget{htmlwidget-5ae7c48ad8bd001e476f}{}
...
\hypertarget{htmlwidget-ad6581fdebfb165264ba}{}
...
Another interesting fact is that, in the above fact, if we also consider number of letters of both text categories and get the single represenation number just like we did in Evidence \ref{EvChapContmult19} in the previous section, then the digit sum of resulting big number is also multiple of 19. Here is the proof:
...
\begin{verbatim}
## The probability of all the  43 success out of all the 98  tests (with p=1/19) is: 6.96366434775727e-29
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  43 success out of all the 98  tests (with p=1/19) and Probability
##           of number 19 being in Quran together is: 6.10847749803269e-31
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  43 success out of all the 196  tests (with p=1/19) is: 1.36722645960393e-15
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  43 success out of all the 196  tests (with p=1/19) and Probability of number 19 
##           being in Quran together is: 1.19932145579292e-17
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  43 success out of all the 392  tests (with p=1/19) is: 5.70656924380136e-06
\end{verbatim}
...
\begin{verbatim}
## The probability of all the  43 success out of all the 392  tests (with p=1/19)  and Probability of number 19
##           being in Quran together is: 5.0057624945626e-08
\end{verbatim}
...
In short, there is no violence at all in Islam regarding Quran but those bad examples are based on some of the claimed hadith reports other consensus. I explained before that I reject all the claimed hadith reports as the source of Islam. They are just historical claimed reports and we can get some historical and thus blurred information about past. We can also utilize them to learn about Arabic language of the past as well. As long as they are not considered as the source of Islam, then those claimed hadith reports can also be studied, \textbf{if wished}, to understand a topic in the religion. If the approach is just like to listen any other \textbf{preferred} scholar's opinion that we might consider to understand a topic in religion, then one can also utilize from them while try to understand a topic in the religion. However, this is a very sensitive case as one might be biased towards the claimed hadith report rather than objectively consider it along with all the other information about a topic. Therefore, be warned that if one has a any bias towards them, then there is always potential to infer incorrect biased conclusion on an issue.
